...
def send_welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        f"Hello there, {message.from_user.first_name}! I'm ready to check your pronunciation! 🤓",
        reply_markup=markup,
    )
...
def main_text_handler(message):
    if message.text in (MSG_CHECK_PRONUN, "/checkpr"):
        ...
        bot.send_message(message.chat.id, HELP, reply_markup=render_main_menu())
        ...
        try:
            ...
            bot.send_message(message.chat.id, "❤️", reply_markup=render_main_menu())
    elif message.text == "/admin":
        bot.send_message(message.chat.id, "Checking your telegram id...")
        check_tg_id(message)
