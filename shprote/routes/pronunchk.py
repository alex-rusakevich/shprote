...
def start_pronun_test(message):
    ...
    bot.send_message(
        message.chat.id,
        tf.format_text(
            f"The *pronunciation* check has started. _Please, remember that you cannot use replied or forwarded messages as student's answers._ Check's special code is",
            tf.mcode(user_hash),
            separator=" ",
        ),
    )
...
def get_teacher_text_and_print_stud(message, user_hash):
    ...
    if message.content_type == "text":
        ...
        bot.send_message(
            message.chat.id,
            tf.format_text(
                f"Teacher wrote {forwarded_msg}: {teacher}", tf.mcode(user_hash)
            ),
        )
...
def get_stud_and_calc_result(message, data):
    ...
    if message.content_type == "voice":
        ...
        bot.send_message(
            message.chat.id,
            f'🔴 Wrong data type ("{message.content_type}"), please, send a _voice_ message:',
            reply_markup=render_stop_test_btn(),
        )
        bot.register_next_step_handler(message, get_stud_and_calc_result, data=data)
        return
    ...
    bot.send_message(
        message.chat.id,
        generate_final_answer("pronunciation", data, check_result),
        reply_markup=markup,
        parse_mode="HTML",
    )
